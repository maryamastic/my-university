  ...
  return (
    <>
      ...
      <section style={{
        ...
        alignItems: "center",
        overflow: "hidden",
        padding: "clamp(4rem, 8vw, 8rem) 0 clamp(2rem, 5vw, 5rem)",
      }}>
      ...
      <section className="section" style={{ background: "#ffffff" }}>
        <style>{`
          @media (max-width: 768px) {
            .master-program-card { grid-template-columns: 1fr !important; }
            .master-modules-grid { grid-template-columns: 1fr !important; }
            .master-info-flex { flex-direction: column; gap: 1rem; }
          }
        `}</style>
        <div className="container">
          <div style={{ display: "flex", flexDirection: "column", gap: "2.5rem" }}>
            {programs.map((p) => (
              <FadeUp key={p.code}>
                <div className="master-program-card" style={{ background: "#f8f4ef", border: "1px solid rgba(201,168,76,0.2)", padding: "clamp(1.5rem, 4vw, 3rem)", display: "grid", gridTemplateColumns: "1fr 1fr", gap: "clamp(1.5rem, 4vw, 3rem)", alignItems: "start" }}>
                  <div>
                    <div style={{ display: "flex", alignItems: "center", gap: "1rem", marginBottom: "1.2rem", flexWrap: "wrap" }}>
                      <span style={{ fontFamily: "Cormorant Garamond, serif", fontSize: "clamp(2rem, 6vw, 3rem)", fontWeight: 700, color: "rgba(232, 78, 15, 0.15)", lineHeight: 1 }}>{p.code}</span>
                    ...
                    <h2 style={{ fontFamily: "Cormorant Garamond, serif", fontSize: "clamp(1.3rem, 3vw, 1.6rem)", fontWeight: 600, color: "#1a1a2e", marginBottom: "1rem", lineHeight: 1.3 }}>{p.title}</h2>
                    <p style={{ fontSize: "clamp(0.8rem, 2vw, 0.9rem)", color: "#5a6070", lineHeight: 1.85, marginBottom: "1.5rem" }}>{p.desc}</p>
                    <div className="master-info-flex" style={{ display: "flex", gap: "2rem", marginBottom: "2rem" }}>
                      <div>
                  ...
                  <div>
                    ...
                    <div className="master-modules-grid" style={{ display: "grid", gridTemplateColumns: "1fr 1fr", gap: "0.75rem" }}>
                      {p.modules.map((mod) => (
